...
we reassign to `psi` the state $|10\rangle$ via the user-defined 
literal `ket operator"" _q()` which transforms strings of zero and ones to
...
or the `mket()` function. In line 18 we declare a
